...
//node.js ㅣ 실무에서 사용하지 않는다.
//expressjs : nodejs 프레임워크를 실무에서 사용한다.
http.createServer((req,resp)=>{
    ...
    if(urlObj.pathname=="/"){
        ...
        console.log(params);
        resp.write("<h1>a 와 b 파라미터를 거듭제곱 하는 동적 페이지</h1>")
        ...
        conn.query("SELECT * FROM DEPT",(err,rows,fields)=>{
            ...
            resp.write(html);
            resp.end();
        })//("실행할 질의", ()=>{쿼리가 실행되고 값이 반환되면 실행되는 콜백함수})
        }catch (e){
            console.error(e);
        }
        ...
        resp.write("<h1>404 없는 주소 입니다.</h1>")
        resp.end();
    }
}).listen(7070);