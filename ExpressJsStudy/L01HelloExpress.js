...
app.get("/",(req,res)=>{
    ...
    //res.send() : content-type 을 자동으로 맵핑하고 res.enㅇ() 자동으로 한다.
    const o={checkId:true};
    //res.send(o); //JSON.stringify(o) content-type application/json
    let html = ("<h1>안녕 Express.js</h1>")
    html+=("<h2>Express.js 는 node.js 의 웹앱 프레임워크(다수의 라이브러리의 의해서 웹앱이 지배당하고 있을 때)입니다.</h2>")
    html+=`<ul>
                <li>nodejs 동적페이지 구분이 안되는 것을 express 가 요청 메소드(GET,POST)와 동일이름의 함수(라우팅)를 제공해서 해결</li>
                <li>한 페이지에 너무 많은 동적 리소스를 작성하는 것을 router 라는 것으로 해결</li>
                <li>middleware 를 이용해서 특정 요청의 중간처리를 할 수 있다.</li>
                <li>미들웨어를 이용해서 편리하게 라이브러리 적용을 할 수 있다.</li>
                <li>node.js 의 모든 기능 사용할 수 있다.</li>
            </ul>
            <h2>라우팅과 res.send()</h2>
            <ul>
            <li><a href="/checkIdJson.do">JSON 페이지 요청</a></li>
            <li><a href="/sum.do?a=10&b=20&c=30">html 페이지 요청(파라미터 처리로 더하기 구현)</a></li>
            <li><a href="/img/참새.png">정적 페이지 요청</a></li>
            <li><a href=""></a></li>
            </ul>
...
app.get("/checkIdJson.do",(rep,res)=>{
